...
class Command(BaseCommand):
    help = 'Registra una compra directo en tesorería (Banco o Dinero Guardado)'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--categoria',
            type=str,
            help='Código de la categoría de compra (ej: EQUIPOS, INVENTARIO, COMPRA)',
        )
        parser.add_argument(
            '--monto',
            type=float,
    ...
    def handle(self, *args, **options):
        ...
        # Mostrar información
        self.stdout.write('🛒 REGISTRAR COMPRA DE TESORERÍA')
        self.stdout.write(f'👤 Usuario: {usuario.get_full_name() or usuario.username}')
        self.stdout.write('=' * 60)
        self.stdout.write('ℹ️  Esta compra se registrará directamente en tesorería.')
        ...
        # Validar que sea un tipo de inversión/compra
        if tipo_movimiento.tipo_base not in ['INVERSION', 'GASTO']:
            raise CommandError(f'❌ "{tipo_movimiento.nombre}" no es un tipo de compra válido')
        ...
        # Mostrar resumen
        self.stdout.write('\n' + '=' * 60)
        self.stdout.write('📊 RESUMEN DE LA COMPRA:')
        ...
        # Confirmación
        if usar_interactivo:
            confirmar = input('\n¿Confirma el registro de esta compra? (s/N): ')
        ...
        try:
            ...
            with transaction.atomic():
                ...
                self.stdout.write(f'📅 Fecha: {fecha_compra_tz.strftime("%d/%m/%Y %H:%M")}')

                self.stdout.write('\n🎉 Compra de tesorería registrada correctamente!')

        except Exception as e:
            raise CommandError(f'❌ Error al crear la compra: {str(e)}')
    ...
    def _modo_interactivo(self):
        ...
        while True:
            try:
                opcion = input(f'\n¿De qué cuenta desea realizar la compra? (1-{cuentas_disponibles.count()}): ')
        ...
        # Selección de categoría de compra/inversión
        self.stdout.write('\n🏷️ CATEGORÍAS DE COMPRA DISPONIBLES:')
        self.stdout.write('-' * 40)
        ...
        if not tipos_compra.exists():
            raise CommandError('❌ No hay categorías de compra configuradas')
        ...
        for idx, tipo in enumerate(tipos_compra, 1):
            if tipo.tipo_base != tipo_actual:
                ...
                emoji = '🔧' if tipo_actual == 'INVERSION' else '💸'
                self.stdout.write(f'\n   {emoji} {tipo_actual}:')
        ...
        while True:
            try:
                opcion = input(f'\n¿Cuál categoría desea usar? (1-{tipos_compra.count()}): ')
                idx_seleccionado = int(opcion) - 1
        ...
        # Descripción
        descripcion = input('\n📝 Descripción de la compra: ').strip()
        if not descripcion:
