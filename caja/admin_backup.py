...
class CajaRegistradoraAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'cajero__username', 'cajero__first_name', 'cajero__last_name')
    ordering = ('-fecha_apertura',)
    readonly_fields = ('fecha_apertura', 'monto_final_sistema', 'diferencia')
    
    def has_add_permission(self, request):
        """
        Solo superusuarios pueden crear cajas desde el admin.
        """
        return request.user.is_superuser
    
    def get_readonly_fields(self, request, obj=None):
        """
        Superusuarios pueden editar más campos.
        Staff y otros usuarios tienen campos readonly.
        """
        if request.user.is_superuser:
            # Superusuarios pueden editar más campos, pero algunos calculados siguen readonly
            return ('fecha_apertura', 'monto_final_sistema', 'diferencia', 'total_ingresos', 'total_egresos', 'duracion_formateada')
    ...
    def get_fields(self, request, obj=None):
        """
        Campos disponibles según el usuario y si es creación o edición.
        """
        if obj is None:  # Creando nueva caja
            # Al crear, solo permitir campos básicos
            return ('cajero', 'monto_inicial', 'observaciones_apertura')
        else:  # Editando caja existente
            # Al editar, mostrar todos los campos
            return (
                'cajero', 'estado', 'fecha_apertura', 'fecha_cierre', 'duracion_formateada',
                'monto_inicial', 'monto_final_declarado', 'monto_final_sistema', 'diferencia',
                'dinero_en_caja', 'dinero_guardado', 'total_ingresos', 'total_egresos',
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if obj is None:  # Creando nueva caja
            return (
                (_('Crear Nueva Caja'), {
                    'fields': ('cajero', 'monto_inicial', 'observaciones_apertura'),
                    'description': '⚠️ IMPORTANTE: Solo crear cajas desde aquí en casos especiales. Lo normal es usar el dashboard del sistema.'
                }),
            )
        else:  # Editando caja existente
            return (
    ...
    def monto_inicial_fmt(self, obj):
        ...
        try:
            if obj.monto_inicial is not None:
    ...
    def monto_final_declarado_fmt(self, obj):
        ...
        try:
            if obj.monto_final_declarado is not None:
    ...
    def monto_final_sistema_fmt(self, obj):
        ...
        try:
            if obj.monto_final_sistema is not None:
